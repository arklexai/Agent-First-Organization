...
class TurnDetection(BaseModel):
    ...
    def model_dump(self) -> dict:
        ...
        Note:
            ...
            For other types, prefix_padding_ms, silence_duration_ms, and threshold are removed.
        """
...
class OpenAIRealtimeAgent(BaseAgent):
    ...
    conversation flow.
    """
    def __init__(self, telephony_mode: bool = False, prompt: str = "", voice: str = "alloy", transcription_language: Optional[str] = None, speed: float = 1.0, turn_detection: TurnDetection | None = None, tool_map: dict[str, Tool] = None, prompt_variables: list[PromptVariable] = []):
        """
        ...
        if prompt_variables:
            ...
            prompt_variables_dict = {
                pv.name: pv.value for pv in prompt_variables
            }
        ...
        self.tool_defs = [tool.to_openai_tool_def() for tool in (tool_map or {}).values()]
        self.transcript = []
    ...
    async def update_session(self) -> None:
        ...
        event = {
            "type": "session.update",
            "session": {
                "turn_detection": self.turn_detection.model_dump() if self.turn_detection else None,
        ...
        if self.transcription_language:
            event["session"]["input_audio_transcription"]["language"] = self.transcription_language
    ...
    async def run_voicemail_tool(self, tool: Tool):
        """
        ...
        logger.info(f"Running voicemail tool with message: {tool.fixed_args['message']}")
    ...
    async def run_tool(self, call_id, tool_name: str, tool_args: dict):
        """
                    ...
                    if schema_obj.get("valueSource", "") == "fixed":
                        for filled_ob in tool_args[slot.name]:
                            if schema_obj.get("type") == "bool":
                                filled_ob[schema_obj.get("name")] = schema_obj.get("value", "").lower() == "true"
                            else:
                                filled_ob[schema_obj.get("name")] = schema_obj.get("value")
        ...
        async for openai_message in self.ws:
            try: 
                ...
                if event_type == 'response.done':
                    ...
                    if openai_event.get("response") and openai_event["response"].get("output"):
                        for output in openai_event["response"]["output"]:
                            if output.get("type") == "function_call":
                                logger.info(f"function call received: {output['name']}")
                                try:
                                    await self.run_tool(output["call_id"], output["name"], json.loads(output["arguments"]))
                                except Exception as e:
                                    logger.error(f"Error running tool {output['name']}: {e}")
                                    logger.exception(e)
                                    raise e
                ...
                if event_type == 'response.audio_transcript.delta' and 'delta' in openai_event and not self.telephony_mode:
