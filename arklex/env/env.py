...
class Env:
    def __init__(
        ...
        if planner_enabled:
            self.planner: ReactPlanner = ReactPlanner(
                tools_map=self.tools, workers_map=self.workers, name2id=self.name2id
            )
        else:
            self.planner: DefaultPlanner = DefaultPlanner(
                tools_map=self.tools, workers_map=self.workers, name2id=self.name2id
            )
    ...
    def step(
        ...
        if id in self.tools:
            ...
            response_state: MessageState = tool.execute(
            ...
            response_state: MessageState = worker.execute(
