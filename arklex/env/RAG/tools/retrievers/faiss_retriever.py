...
class FaissRetrieverExecutor:
    ...
    def _init_retriever(self, **kwargs: Any) -> Any:
        ...
        docsearch: FAISS = FAISS.from_documents(
            self.texts, self.embedding_model)
    ...
    def search(
        ...
        docs_and_score: List[Tuple[Document, float]
                             ] = self.retrieve_w_score(ret_input)
        retrieved_text: str = ""
        retriever_returns: List[Dict[str, Any]] = []
    ...
    def load_docs(
        ...
        document_path: str = os.path.join(
            database_path, "chunked_documents.pkl")
