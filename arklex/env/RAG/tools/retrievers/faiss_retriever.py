...
class RetrieveEngine():
    ...
    def faiss_retrieve(state: MessageState):
        ...
        docs = FaissRetrieverExecutor.load_docs(database_path=os.environ.get(
            "DATA_DIR"), llm_config=state.bot_config.llm_config)
...
class FaissRetrieverExecutor:
    ...
    def retrieve_w_score(self, query: str):
        k_value = 4 if not self.retriever.search_kwargs.get(
            'k') else self.retriever.search_kwargs.get('k')
