...
class NegotiationMultiIssueWorker(BaseWorker):
    ...
    def generate_combos(self, sample_size: int = 30) -> str:
        ...
        valid_combinations = [
            {
                issue: choice[0]
                for issue, choice in zip(
                    self.payoff_schedule.keys(), combination, strict=False
                )
            }
    ...
    def update_offers_and_kde(
        self, issue: str, current_offer: float, previous_offer: float
    ) -> float:
        ...
        if (
            len(self.observations[issue]) > 1
        ):  # Fit KDE only if there are at least two observations
            self.kde_models[issue].fit(
                np.array(self.observations[issue]).reshape(-1, 1)
            )
        else:
            return -1  # Not enough data yet to calculate the peak

        # Step 3: Evaluate the density over a range of possible concession sizes
        concession_range = np.linspace(
            min(self.observations[issue]), max(self.observations[issue]), 1000
        ).reshape(-1, 1)
    ...
    def get_response(self, state: MessageState) -> MessageState:
        ...
        if current_turn == 0:
            ...
            prompt_path = os.path.join(
                base_dir, "negotiation_prompts", "adaptedscenario.txt"
            )
            ...
            if current_status:
                pattern = (
                    r"ISSUE:\s*(.*?)\s*CURRENT OFFER:\s*(\d+)\s*PREVIOUS OFFER:\s*(\d+)"
                )
                ...
                if match:
                    ...
                    if (
                        issue not in ["LOCATION", "JOB_ASSIGNMENT"]
                        and current_offer
                        and previous_offer
                    ):
                        kde_peak = self.update_offers_and_kde(
                            issue, float(current_offer), float(previous_offer)
                        )
                        if kde_peak != -1:
                            ...
                            state.slots["current_target"][0].value = json.dumps(
                                current_target
                            )
                            state.message_flow += (
    ...
    def monitor_instance(
        self, state: MessageState, issues: list[str]
    ) -> dict[str, str] | None:
        ...
        if "NUMBER OF RESOLVED ISSUES=6" in monitor_response:
            ...
            final_outcome_start = monitor_response.index(
                "NUMBER OF RESOLVED ISSUES=6"
            ) + len("NUMBER OF RESOLVED ISSUES=6")
