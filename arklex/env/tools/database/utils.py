...
class DatabaseActions:
    ...
    def init_slots(self, slots: List[Slot], bot_config: Any) -> List[Dict[str, str]]:
        if not slots:
            slots = SLOTS
        self.slots = []
        self.slot_prompts = []
    ...
    def verify_slot(
        self, slot: Slot, value_list: List[Any], bot_config: Any
    ) -> SlotDetail:
    ...
    def cancel_booking(self, msg_state: MessageState) -> MessageState:
        ...
        # Check whether info is enough to cancel a booking
        if len(rows) > 1:
            msg_state.status = StatusEnum.INCOMPLETE
            if self.slot_prompts:
                msg_state.message_flow = self.slot_prompts[0]
            ...
            booking_id: str = results[0]["id"]
            cursor.execute("DELETE FROM booking WHERE id = ?", (booking_id,))
            msg_state.status = StatusEnum.COMPLETE
            msg_state.message_flow = "The booking has been cancelled."
        cursor.close()
        conn.close()
        return msg_state
