...
class Tool:
    ...
    def to_openai_agents_function_tool(self) -> "FunctionTool":
        ...
        async def on_invoke(ctx: RunContextWrapper[Any], raw_args: str) -> str:
            ...
            try:
                ...
                # Apply fixed values from schemas FIRST (so they can't be overridden)
    ...
    def _apply_schema_fixed_values(self) -> None:
        ...
        try:
            # Apply fixed values directly to slots based on valueSource
            for slot in self.slots:
                value_source = getattr(slot, "valueSource", "prompt")
            ...
            # Apply fixed/default values to slots with schema (for nested slots)
            for slot in self.slots:
