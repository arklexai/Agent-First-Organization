...
def register_tool(desc, slots=[], outputs=[], isResponse=False):
    ...
    def inner(func):
        ...
        def tool(): return Tool(func, key, desc, slots, outputs, isResponse)
        return tool
    return inner
...
class Tool:
    ...
    def get_info(self, slots):
        self.properties = {}
        for slot in slots:
            self.properties[slot["name"]] = {k: v for k, v in slot.items(
            ) if k in ["type", "description", "prompt", "items"]}
        required = [slot["name"]
                    for slot in slots if slot.get("required", False)]
        return {
            "type": "function",
            "function": {
                "name": self.name,
    ...
    def _execute(self, state: MessageState, **fixed_args):
        ...
        if self.isResponse and tool_success:
            logger.info(
                "Tool output is stored in response instead of message flow")
            state.response = response
        else:
            state.message_flow = state.message_flow + \
