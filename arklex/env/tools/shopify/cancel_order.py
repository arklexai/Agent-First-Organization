...
def cancel_order(cancel_order_id: str, **kwargs: Any) -> str:
    ...
    try:
        with shopify.Session.temp(**auth):
            ...
            """)
            response_data: Dict[str, Any] = json.loads(response)["data"]
            if not response_data.get("orderCancel", {}).get("userErrors"):
                return "The order is successfully cancelled. " + json.dumps(
                    response_data
                )
            else:
                raise ToolExecutionError(
                    func_name, json.dumps(response_data["orderCancel"]["userErrors"])
                )
