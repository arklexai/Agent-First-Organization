...
class SlotFiller(BaseSlotFilling):
    ...
    def _slots_to_openai_schema(self, slots: list[Slot]) -> dict[str, Any]:
        ...
        for slot in slots:
            # Skip fully-fixed (non-nested) slots from going to the LLM
            # These will be filled after the model response from their fixed/default values
    ...
    def _remove_non_openai_fields(self, schema_obj: dict | list) -> None:
        ...
        if isinstance(schema_obj, dict):
            # Remove non-OpenAI fields (keep 'default' so LLM sees defaults)
            fields_to_remove = [
                "valueSource", "fixed", "prompt", 
    ...
    def _apply_fixed_values_recursive(self, item: dict, properties: dict, path: str = "") -> dict:
        ...
        for field_name, field_schema in properties.items():
            ...
            # Check if this field has a fixed or default value
            if field_schema.get("valueSource") == "fixed" and "value" in field_schema:
                # Fixed always overrides the model output
    ...
    def verify_slot(
        ...
        try:
            ...
            # Best-effort guard; fall through to normal flow on any error
            pass
