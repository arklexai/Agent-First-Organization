...
class NLUModelAPI:
    ...
    def get_response(
        self, sys_prompt, model, response_format="text", note="intent detection"
    ):
    ...
    def predict(self, text, intents, chat_history_str, model) -> str:
        system_prompt, idx2intents_mapping = self.format_input(
            intents, chat_history_str
        )
        response = self.get_response(system_prompt, model, note="intent detection")
...
class SlotFillModelAPI:
    ...
    def predict(self, slots: list[Slot], input: str, model: dict, type: str = "chat"):
        try:
