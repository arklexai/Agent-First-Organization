...
def get_relevant_records(state: MessageState, query: str, top_k: int = 3, threshold: float = 0.7) -> Union[List[ResourceRecord], str]:
    """
    Retrieve top-k relevant memory records with weighted similarity score ≥ threshold.
    
    Weights (equal): Task, Intent, Context Generate, Output, Recency → 0.2 each
    """
    ...
    for turn_idx, turn in enumerate(recent_turns):
        recency_score = (turn_idx + 1) / total_turns  # More recent = higher
    ...
    # Filter and return top-k if any pass the threshold
...
def get_most_relevant_intent(state: MessageState, query: str, threshold: float = 0.7) -> Union[str, None]:
    """
    Compare the query with `intent` fields from last 5 turns.
    Returns the most similar intent if similarity ≥ threshold, else 'not in context'.
    """
    if not state.trajectory:
        return "not in context"
