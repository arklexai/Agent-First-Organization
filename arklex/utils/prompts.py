...
def load_prompts(language: str) -> dict[str, str]:
    ...
    if language.upper() == "EN":
        ...
        prompts = {
            # ===== vanilla prompt ===== #
            "generator_prompt_system": """{sys_instruct}
...
Context:
{context}""",
            "context_generator_prompt_speech_system": """{sys_instruct}
...
{context}""",
            # ===== message prompt ===== #
            "message_generator_prompt_system": """{sys_instruct}
...
{message}""",
            "message_generator_prompt_speech_system": """{sys_instruct}
...
{message}""",
            ...
            "message_flow_generator_prompt_system": """{sys_instruct}
...
In addition to replying to the user, also incorporate the following message into the response naturally if it is not None and doesn't conflict with the original response, the response should be natural and human-like: 
{message}""",
            "message_flow_generator_prompt_speech_system": """{sys_instruct}
...
Answer with only 'yes' or 'no'""",
            ...
            "answer_node_prompt_with_context_system": """{sys_instruct}
...
IMPORTANT: Respond directly to the user's question based on the task and context provided. Do not give generic responses.""",
            "answer_node_prompt_with_context": """Conversation history:
{history}

Context from previous operations:
{context}""",
            "answer_node_prompt_without_context_system": """{sys_instruct}
...
IMPORTANT: Respond directly to the user's question based on the task provided. Do not give generic responses.""",
            "answer_node_prompt_without_context": """Conversation history:
{history}""",
        }
...
重要提示：请根据提供的任务和上下文直接回答用户的问题。不要给出通用回复。""",
            "answer_node_prompt_with_context": """对话历史：
{history}
...
重要提示：请根据提供的任务直接回答用户的问题。不要给出通用回复。""",
            "answer_node_prompt_without_context": """对话历史：
{history}""",
        }
    else:
