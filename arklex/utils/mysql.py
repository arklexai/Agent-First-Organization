...
class MySQLPool(object):
    def __init__(self, pool_size: int, **kwargs: Any) -> None:
        ...
        self.dbconfig: Dict[str, Union[str, int]] = {
            "host": self._host,
            "port": self._port,
            "user": self._user,
    ...
    def get_connection(self) -> mysql.connector.pooling.PooledMySQLConnection:
        ...
        while time.time() - t0 < CONNECTION_TIMEOUT:
            try:
                conn: mysql.connector.pooling.PooledMySQLConnection = (
    ...
    def execute(self, sql: str, params: Optional[Tuple[Any, ...]] = None) -> None:
        conn: mysql.connector.pooling.PooledMySQLConnection = self.get_connection()
        try:
...
mysql_pool: MySQLPool = MySQLPool(POOL_SIZE, **MYSQL_CONFIG)
